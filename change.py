...
def doDiff(img,tar_color,skin_color,size): # tar_color =RGB
	print("tar_color: {}, skin_color: {}".format(tar_color, skin_color))
		...
		for j in range(size[1]):
			for k in range(3):
				...
				if(img[i,j,k]<skin_color[k]): 
					img[i,j,k] = img[i,j,k] * tar / src 
					if img[i,j,k] >= 255: img[i,j,k] = 255
				else:
					img[i,j,k] = tar + ((255-tar) * (img[i,j,k] - src) / (255-src))
					if img[i,j,k] >= 255: img[i,j,k] = 255
...
def change_skin(image_file, tar_color, img_path, seg_path, mode): 
	...
	skin_color = get_skin_color(img_path, seg_path, mode) # HSV
...
#	if(skinRange(skin_color[hsvskin_color[1],skin_color[2])): 
	...
	_want_sum = _tar_color[0] + _tar_color[1] + _tar_color[2]
	_skin_sum = _skin_color[0] + _skin_color[1] + _skin_color[2]
